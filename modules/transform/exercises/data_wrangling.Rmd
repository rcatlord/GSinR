...
We could have also used `if_else` for this, which is the tidyverse version of `ifelse` (somewhat confusingly). They behave very similar but `if_else` is a bit [more strict](https://stackoverflow.com/questions/50646133/dplyr-if-else-vs-base-r-ifelse). We use `if_else` a bit in the course, so you'll get to try it out.
