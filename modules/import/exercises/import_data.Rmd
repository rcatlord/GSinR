...
To get the file path, there are a variety of tricks you can do. The simplest thing is just to save your file into your working directory. In this case, you only have to specify the file name, and R will know to look in your working directory. So if I have a file in my working directory called `myCsv.csv`, then I can make the path the file name, like so: 


```{r, eval=FALSE}
my_csv_path <- "myCsv.csv"
```
...
```{r}
my_csv_path <- "/Users/reka/Desktop/course-incubator/data/myCsv.csv"
```
